...
void ButterComp2Plugin::processReplacing(float **inputs, float **outputs, VstInt32 sampleFrames) 
{
    float* in1  =  inputs[0];
    float* in2  =  inputs[1];
    ...
    while (--sampleFrames >= 0)
    {
		...
		double cur_min_gain = (totalmultiplierL < totalmultiplierR ? totalmultiplierL : totalmultiplierR);
	...
	set_min_gain(min_gain, num_samples);
}

void ButterComp2Plugin::processDoubleReplacing(double **inputs, double **outputs, VstInt32 sampleFrames) 
{
    double* in1  =  inputs[0];
    double* in2  =  inputs[1];
