...
void Dynamics::processReplacing(float **inputs, float **outputs, VstInt32 sampleFrames) 
{
    ...
    while (--sampleFrames >= 0)
    {
		...
		inputSampleL /= inputgain;
		//end L
		double cur_min_gain = totalmultiplier;
		...
		flip = !flip;
		//end compressor
		if (totalmultiplier < cur_min_gain)
			cur_min_gain = totalmultiplier;
		...
		if (gaterollerL < 1.0)
		{
			...
			if (gateL < cur_min_gain) cur_min_gain = gateL;
		} else gateL = 1.0;
		...
		if (gaterollerR < 1.0)
		{
			...
			if (gateR < cur_min_gain) cur_min_gain = gateR;
		} else gateR = 1.0;
		//end Gate
...
void Dynamics::processDoubleReplacing(double **inputs, double **outputs, VstInt32 sampleFrames) 
{
    ...
    while (--sampleFrames >= 0)
    {
		...
		flip = !flip;
		//end compressor
		if (totalmultiplier < cur_min_gain) cur_min_gain = totalmultiplier;
