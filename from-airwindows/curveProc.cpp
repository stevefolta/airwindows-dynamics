...
void curvePlugin::processReplacing(float **inputs, float **outputs, VstInt32 sampleFrames) 
{
    float* in1  =  inputs[0];
    float* in2  =  inputs[1];
    ...
    while (--sampleFrames >= 0)
    {
		...
		if (gain < 0.0078125) gain = 0.0078125;
	...
	set_min_gain(min_gain, num_frames);
}

void curvePlugin::processDoubleReplacing(double **inputs, double **outputs, VstInt32 sampleFrames) 
{
    double* in1  =  inputs[0];
    double* in2  =  inputs[1];
